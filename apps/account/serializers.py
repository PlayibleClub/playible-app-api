    ...
    def validate(self, data):
        try:
            ...
            raise serializers.ValidationError(
                'Failed to query contract information from the provided contract address.')
        return data
...
class AssetSerializer(serializers.ModelSerializer):
    ...
    def get_token_info(self, obj):
        if self.instance is not None:
            token_info = terra.query_contract(getattr(obj, "collection").contract_addr, {
                                              "nft_info": {"token_id": getattr(obj, "token_id")}})
        else:
            token_info = terra.query_contract(obj["collection"].contract_addr, {
                                              "nft_info": {"token_id": obj["token_id"]}})
        return token_info

    def validate(self, data):
        try:
            owner_info = terra.query_contract(data["collection"].get('contract_addr'), {
                                              "owner_of": {"token_id": data["token_id"]}})
        except:
            raise serializers.ValidationError(
                'Failed to query token information from the provided contract address with the provided token ID.')
